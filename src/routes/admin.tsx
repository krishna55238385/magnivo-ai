...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [{ title: "Admin — Magnivo.ai" }, { name: "robots", content: "noindex,nofollow" }],
  }),
  component: AdminPage,
});
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="surface-card p-8 max-w-md text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Your account doesn't have the admin role. Ask an existing admin to grant access for user
            id:
          </p>
          <code className="mt-3 block text-xs font-mono break-all p-2 rounded bg-card border border-border">
            {user.id}
          </code>
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container-x h-14 flex items-center justify-between gap-3">
          <Link to="/" className="flex items-center gap-2 font-bold">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-border bg-card">
              <span className="text-[13px] font-bold tracking-tight bg-gradient-to-br from-[var(--accent-blue)] to-[var(--accent-green)] bg-clip-text text-transparent">
                M
              </span>
            </span>
      ...
      <main className="container-x py-8 md:py-10">
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          ...
          <SegBtn active={filter === "demo"} onClick={() => setFilter("demo")}>
            Demo
          </SegBtn>
          <SegBtn active={filter === "contact"} onClick={() => setFilter("contact")}>
            Contact
          </SegBtn>
          <span className="mx-2 text-border">|</span>
          <SegBtn active={statusFilter === "all"} onClick={() => setStatusFilter("all")}>
            All status
          </SegBtn>
          <SegBtn active={statusFilter === "new"} onClick={() => setStatusFilter("new")}>
            New
          </SegBtn>
          <SegBtn
          ...
          <SegBtn active={statusFilter === "closed"} onClick={() => setStatusFilter("closed")}>
            Closed
          </SegBtn>
        </div>
                ...
                <li key={s.id}>
                  ...
                  >
                    <div className="flex items-center gap-3 md:w-64 shrink-0">
                      <span className={s.type === "demo" ? "pill-blue" : "pill-green"}>
                        {s.type}
                      </span>
        ...
        <div className="fixed inset-0 z-40" role="dialog" aria-modal="true">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-full max-w-lg bg-card border-l border-border p-6 overflow-y-auto animate-in slide-in-from-right">
            <div className="flex items-center justify-between">
              <span className={active.type === "demo" ? "pill-blue" : "pill-green"}>
                {active.type}
              </span>
              <button
                onClick={() => setActive(null)}
...
function StatusDot({ status }: { status: Submission["status"] }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-widest text-muted-foreground">
      <span className="h-1.5 w-1.5 rounded-full" style={{ background: map[status] }} />
      {status.replace("_", " ")}
    </span>
  );
}
